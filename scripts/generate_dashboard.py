...
<div class="max-w-7xl mx-auto p-4 space-y-4">
  ...
  <!-- Open Positions (rows injected by JS; live price + unrealised P&L filled by refreshPrices) -->
  <div class="bg-gray-800 rounded-xl p-4">
  ...
  <p class="text-center text-xs text-gray-600 pb-2">Paper trading only &middot; Alpaca Markets &middot; Data refreshes every ~15 min &middot; Prices live every 60 s</p>
</div>
...
if (last) {
  const el = $('c-today-pnl');
  el.textContent = sign$(last.pnl);
  el.className = 'text-xl font-bold ' + pnlCls(last.pnl);
}
...
if (daily.length > 0) {
  new Chart($('equity-chart'), { type: 'line', data: { labels: daily.map(d => d.date),
    datasets: [{ data: daily.map(d => parseFloat(d.ending_equity)), borderColor: '#4ade80',
      backgroundColor: 'rgba(74,222,128,0.07)', borderWidth: 2, fill: true, tension: 0.3,
      pointRadius: daily.length > 30 ? 0 : 3 }] }, options: cd });
} else { $('eq-empty').classList.remove('hidden'); $('equity-chart').remove(); }

if (daily.length > 0) {
  const pnls = daily.map(d => parseFloat(d.pnl));
  new Chart($('pnl-chart'), { type: 'bar', data: { labels: daily.map(d => d.date),
    datasets: [{ data: pnls, backgroundColor: pnls.map(v => v >= 0 ? 'rgba(74,222,128,0.75)' : 'rgba(248,113,113,0.75)'),
      borderRadius: 3 }] }, options: cd });
} else { $('pnl-empty').classList.remove('hidden'); $('pnl-chart').remove(); }

// Positions table — static skeleton; live price cells get id="lp-{sym}" / "up-{sym}"
...
async function fetchLivePrices() {
  ...
  // Equities: Yahoo Finance (best-effort; CORS varies by browser)
  try {
    const r = await fetch(
...
async function refreshPrices() {
  ...
  let anyLive = false;
  for (const sym of Object.keys(D.positions)) {
    const safeSym = sym.replace('/', '_');
